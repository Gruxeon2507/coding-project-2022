...
function NPCManager() return MODULE.Get("standardcombo.NPCKit.NPCManager") end
...
function ApplyDamage(attackData)
	...
	-- Prevent npc hang bug.
	--if not NPCManager().IsRegistered(ROOT:FindDescendantByName("NPCAIServer")) then return end
	if currentTime + 3 >= os.time() then return end
