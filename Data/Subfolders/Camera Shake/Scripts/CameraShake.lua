...
function Tick(deltaTime)
	if (amplitude > 0) then
		...
		local ry = Rotation.New(0, pitch, 0)
		local yaw = math.sin(time() * FREQUENCY) * amplitude
		local rx = Rotation.New(yaw, 0, 0)
		camera:SetRotationOffset(ry)
		camera:SetRotationOffset(rx)
...
Events.Connect("Camera Shake", StartShake)