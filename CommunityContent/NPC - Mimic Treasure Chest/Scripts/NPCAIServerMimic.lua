...
local MIMIC_ENABLE = ROOT:GetCustomProperty("MimicEnable")
...
local mimicFlag = true

function SetState(newState)
	--print("NewState = " .. newState)
	if MIMIC_ENABLE == false then
...
function Tick(deltaTime)
	...
	if mimicFlag == true then
		if ROOT:GetCustomProperty("MoveSpeed") > 0 then
			MOVE_SPEED = ROOT:GetCustomProperty("MoveSpeed")
			PATHING_STEP = MOVE_SPEED * LOGICAL_PERIOD + 10
			PATHING_STEP_SQUARED = PATHING_STEP * PATHING_STEP
			ATTACK_RANGE = ROOT:GetCustomProperty("AttackRange")
