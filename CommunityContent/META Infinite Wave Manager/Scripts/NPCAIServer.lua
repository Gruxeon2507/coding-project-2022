...
local DEAD_1_DURATION = 1
local DEAD_2_DURATION = 1
...
function RootLookAtContinuous(targetObj, lockPitch, speed)
	--CROSS_CONTEXT_CALLER().Call(function()
	if targetObj and targetObj.isServerOnly and targetObj.parent and 
	not targetObj.parent.isServerOnly then
		targetObj = targetObj.parent
	end
	
	if Object.IsValid(targetObj) and (targetObj:IsA("CoreObject") or targetObj:IsA("Player")) then
		ROTATION_ROOT:LookAtContinuous(targetObj, lockPitch, speed)
	elseif Object.IsValid(targetObj) then
